...
public final class CarInforClient {
    ...
    /**
     * nName Car name nCarID Car ID nCarType Car type,include type,color nStatus
     * Current status of this car normal,accident,idle,login,collision,logout
     * nDirection Current Direction nSpeed Current Speed nAcceleration Current
     * Acceleration nXPosition Current X Position nYPosition Current Y Position
     * nAccidenceSpeed Collision speed nAccidenceAcceleration Collision
    ...
    public boolean draw(GL10 gl) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (nSpeed > 0.0f) {
                gl
                        .glRotatef((float) Math.toDegrees(rotateAngle * 2.5),
                                0, 1, 0);
                gl
                        .glRotatef((float) Math.toDegrees(rotateAngle * 0.5),
                                0, 0, 1);
            } else if (nSpeed < 0.0f) {
                gl.glRotatef((float) Math.toDegrees(-rotateAngle * 2.5), 0, 1,
                        0);
                gl.glRotatef((float) Math.toDegrees(-rotateAngle * 0.5), 0, 0,
                        1);
            }
    ...
    public static final float CAR_CENTER_Y = 0.0f; // the Y coordinate of car's
                                                   // center
    ...
    public void updateDirectionBySensor(float changeDirection, float timeElapsed) {
        if (mDirectionSensorState != NO_SENSOR_EVENT
                && mDirectionKeyState == NO_KEY_EVENT) {
    ...
    public void updateSpeedByKeyboard(int timeElapsed) {
        ...
        if (mSpeedKeyState == SPEED_UP_KEYBOARD) {
            ...
            if (nSpeed > -PRECISION) {
                nAcceleration = -timeElapsed * TOP_ACCELERATION; // slow down
                                                                 // faster if
                                                                 // the speed is
                                                                 // greater than
                                                                 // zero
        ...
        if (mSpeedKeyState != NO_KEY_EVENT) {
            nSpeed += timeElapsed * nAcceleration; // can not put this out of
    ...
    public void updateDirectionByKeyboard(int timeElapsed) {
        ...
        if (mDirectionKeyState == DIRECTION_LEFT_KEYBOARD) {
            if (nSpeed > PRECISION) {
                ...
                // prevent the car from keeping rotating even if the speed is
                // zero
            ...
            if (nSpeed > PRECISION) {
                ...
                nChangeDirection = 0.0f; // prevent the car from keeping
                                         // rotating even if the speed is zero
            }
        }
        // }}changeDirection-----------------------
        ...
        if (mDirectionKeyState != NO_KEY_EVENT) {
            ...
            nDirection += nChangeDirection; // can not get this out of this if
                                            // statement
            if (nDirection < 0) {
    ...
    public void weakOff(int timeElapsed) {
        ...
        if (mSpeedKeyState == NO_KEY_EVENT)// slow down if no event from the
                                           // sensor or keyboard
        {
            nAcceleration = 0.0f; // the sensor's value would not evaluate to
                                  // nAcceleration
            if (mSpeedSensorState == NO_SENSOR_EVENT) {
                if (nSpeed > PRECISION) {
        ...
        if (mDirectionKeyState == NO_KEY_EVENT
                && mDirectionSensorState == NO_SENSOR_EVENT)// stop rotating if
                                                            // no event from the
                                                            // sensor or
                                                            // keyboard
        {
            // nChangeDirection = 0.0f; //prevent the car from keeping rotating
    ...
    public void transform(GL10 gl) {
        ...
        if (nSpeed > 0.1f) {
            if (mDirectionKeyState != NO_KEY_EVENT
                    || mDirectionSensorState != NO_SENSOR_EVENT) {
                gl.glRotatef((float) Math.toDegrees(nChangeDirection * 3), 0,
                        1, 0);
                gl.glRotatef((float) Math.toDegrees((mWheelRollDirection++)
                        + nSpeed), 1, 0, 0);
            } else {
