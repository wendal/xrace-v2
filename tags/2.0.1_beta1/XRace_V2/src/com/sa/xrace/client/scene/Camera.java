...
public class Camera {
    ...
    public void setCamera(GL10 gl) {
        GLU.gluLookAt(gl, mEye.x, mEye.y, mEye.z, mCenter.x, mCenter.y,
                mCenter.z, mUp.x, mUp.y, mUp.z);
    }
    ...
    // update the camera according to the car's information given by the
    // parameters
    public void updateCamera(Point3f myCenter, float myDirection,
            float changeDirection, float speed, int speedKeyState,
            int directionKeyState, int speedSensorState,
            int directionSensorState, int lookDirection) {
        // mDirection = myDirection;
        // {{update distance between camera and car-----------------------
        if (speedKeyState == CarInforClient.SPEED_UP_KEYBOARD
                || speedSensorState == CarInforClient.SPEED_UP_SENSOR) {
            if (mDistance < DataToolKit.FAR_DISTANCE) {
                mDistance += DataToolKit.DISTANCE_PER_FRAME;
            }
        } else if (speedKeyState == CarInforClient.SPEED_DOWN_KEYBOARD
                || speedSensorState == CarInforClient.SPEED_DOWN_SENSOR) {
            if (mDistance > DataToolKit.NEAR_DISTANCE) // nearest distance
            {
                mDistance -= DataToolKit.DISTANCE_PER_FRAME;
            }
        } else if (speedKeyState == CarInforClient.NO_KEY_EVENT
                && speedSensorState == CarInforClient.NO_SENSOR_EVENT) {
        ...
        if (directionKeyState == CarInforClient.DIRECTION_LEFT_KEYBOARD
            ...
            if (speed > DataToolKit.PRECISION) {
                if (mOffset < DataToolKit.MAX_OFFSET
                        && changeDirection > DataToolKit.DIRECTION_PER_FRAME) {
                    mChangeDirection = changeDirection
                            - DataToolKit.DIRECTION_PER_FRAME;
                    mOffset++;
                } else {
                    mChangeDirection = changeDirection;
                }
                ...
                // prevent the camera from keeping rotating even if the speed is
                // zero
                mChangeDirection = 0;
            }
        ...
        } else if (directionKeyState == CarInforClient.NO_KEY_EVENT
                && directionSensorState == CarInforClient.NO_SENSOR_EVENT
                || speed < 0.2f && speed > -0.2f) {

            mChangeDirection = 0;
            if (mOffset > 0) {
                updateCamera(myCenter, mDirection
                        - DataToolKit.DIRECTION_PER_FRAME * (mOffset % 2 - 2));
                mLookDirection = (float) (mDirection
                        - DataToolKit.DIRECTION_PER_FRAME * (mOffset % 2 - 2) + Math.PI);
                mOffset += (mOffset % 2 - 2);
            } else if (mOffset < 0) {
                updateCamera(myCenter, mDirection
                        - DataToolKit.DIRECTION_PER_FRAME * (mOffset % 2 + 2));
