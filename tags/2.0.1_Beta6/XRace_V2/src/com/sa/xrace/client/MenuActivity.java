...
//        Log.e("-----------Menu", "Here");
     ...
     */
    private void startMove(ImageView b, int n, float startX, float toX,
    ...
    /*
     * The class is used to implements Animation.AnimationListener,that when the
     * Animation move end the class will run
     */
    private class DisplayNextButton implements Animation.AnimationListener {
        ...
        public void onAnimationEnd(Animation animation) {
            switch (index) {
            case 0: {
                single.setLayoutParams(new AbsoluteLayout.LayoutParams(b_width,
                        b_height, src_width - 150, src_height / 3));
                single.setVisibility(View.VISIBLE);
                startMove(multiple, 1, src_width, src_width - 170,
                        src_height / 3 + 60, src_height / 3 + 60);
                break;
            }
            case 1: {
                multiple
                        .setLayoutParams(new AbsoluteLayout.LayoutParams(
                                b_width, b_height, src_width - 170,
                                src_height / 3 + 60));
                multiple.setVisibility(View.VISIBLE);
                startMove(setting, 2, src_width, src_width - 190,
                        src_height / 3 + 120, src_height / 3 + 120);
                break;
            }
            case 2: {
                setting.setLayoutParams(new AbsoluteLayout.LayoutParams(
                        b_width, b_height, src_width - 190,
                        src_height / 3 + 120));
    ...
    protected Dialog onCreateDialog(int id) {
        return new AlertDialog.Builder(MenuActivity.this).setIcon(
                R.drawable.alert_dialog_icon).setTitle("Connect Setting")
                .setView(layout).setPositiveButton(R.string.alert_dialog_ok,
                        new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog,
                                    int whichButton) {
                                NetworkToolKit.NAME = cutNames(edit_Name
                                        .getText().toString());
                                NetworkToolKit.SERVERIP = edit_IP.getText()
                                        .toString();
